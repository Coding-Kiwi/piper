...
class VitsModel(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        # REQUIRED FOR LIGHTNING 2.0+ with GANs
    ...
    def _load_datasets(self, validation_split, num_test_examples, max_phoneme_ids):
        if self.hparams.dataset is None:
            return
        full_dataset = PiperDataset(self.hparams.dataset, max_phoneme_ids=max_phoneme_ids)
    ...
    def training_step_g(self, batch: Batch):
        x, x_lengths, y, _, spec, spec_lengths, speaker_ids = (
            batch.phoneme_ids, batch.phoneme_lengths, batch.audios,
            batch.audio_lengths, batch.spectrograms, batch.spectrogram_lengths,
            batch.speaker_ids
        )
        
        y_hat, l_length, _attn, ids_slice, _x_mask, z_mask, (_z, z_p, m_p, logs_p, _m_q, logs_q) = self.model_g(
    ...
    def training_step_d(self, batch: Batch):
        ...
        with autocast(device_type=device_type, enabled=False):
            ...
            self.log("loss_disc_all", loss_disc, prog_bar=True)
            return loss_disc

    def validation_step(self, batch: Batch, batch_idx: int):
        # In validation, we just calculate the losses without optimization
        loss_g = self.training_step_g(batch)
        loss_d = self.training_step_d(batch)
        val_loss = loss_g + loss_d
        self.log("val_loss", val_loss, sync_dist=True)

        # Generate audio examples only for the first batch in validation
        if batch_idx == 0 and self._test_dataset:
            for utt_idx, test_utt in enumerate(self._test_dataset):
                if utt_idx >= self.hparams.num_test_examples:
                    break
                ...
                tag = test_utt.text or f"example_{utt_idx}"
                self.logger.experiment.add_audio(tag, test_audio.cpu(), sample_rate=self.hparams.sample_rate, global_step=self.global_step)
    ...
    def configure_optimizers(self):
        ...
        opt_d = torch.optim.AdamW(
            self.model_d.parameters(), lr=self.hparams.learning_rate,
            betas=self.hparams.betas, eps=self.hparams.eps
        )
        sch_g = torch.optim.lr_scheduler.ExponentialLR(opt_g, gamma=self.hparams.lr_decay)
        sch_d = torch.optim.lr_scheduler.ExponentialLR(opt_d, gamma=self.hparams.lr_decay)
        return [opt_g, opt_d], [sch_g, sch_d]

    def train_dataloader(self):
        return DataLoader(self._train_dataset, collate_fn=UtteranceCollate(self.hparams.num_speakers > 1, self.hparams.segment_size),
                          num_workers=self.hparams.num_workers, batch_size=self.hparams.batch_size, shuffle=True)

    def val_dataloader(self):
        return DataLoader(self._val_dataset, collate_fn=UtteranceCollate(self.hparams.num_speakers > 1, self.hparams.segment_size),
